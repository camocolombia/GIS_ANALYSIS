...
taxa_sub<-taxa_sub[order(taxa_sub$Dormancy_cat== "Very low",taxa_sub$Dormancy_cat== "Low",taxa_sub$Dormancy_cat== "Medium",taxa_sub$Dormancy_cat== "High",decreasing = T),]
  colnames(taxa_sub)<-c("Dormancy_cat","color","value")

  subcat<-as.data.frame(tapply(taxa_sub$Dormancy_cat,taxa_sub$Dormancy_cat,length))
  subcat[,2]<-as.numeric(subcat[,1])
  subcat[,1]<-row.names(subcat)
  colnames(subcat)<-c("category","count")
  for(k in 1:nrow(subcat)){
    if(subcat[k,2]>0 & subcat[k,2]<5){
             taxa_sub[which(taxa_sub$Dormancy_cat==subcat[k,1]),]<-NA
    }else{ cat("processing...", as.character(taxa[[j]])," and ",as.character(colnames(csv_file)[i]),"\n")
        }
    };rm(k)
  taxa_sub<-taxa_sub[!is.na(taxa_sub$Dormancy_cat),]   
  ...
  #colnames(csv_file)[i]
    prom<-ggplot(taxa_sub, aes(x =Dormancy_cat, y = value,fill=Dormancy_cat)) +
      geom_boxplot()+
   ...
   #  scale_fill_manual(labels = c("Very low","Low","Medium","High"),values = c("#0035ff","#00ff19","#faff00","#e60b00"))+
      ...
      theme(panel.background = element_rect(fill = "gray90"),text=element_text(size=36),axis.text.x  =element_text(size=36),axis.text.y  = element_text(size=36,colour="black"),legend.position="none")
 
       ggsave(paste0(out_dir,"/","PDF","/",taxon3,"_",as.character(colnames(csv_file)[i]),"_",Sys.Date(),".pdf"),units="in",width=7,height=7.5,scale=2,dpi=600)
